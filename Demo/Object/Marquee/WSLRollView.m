...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self == [super initWithFrame:frame]) {
        ...
        self.titleLabel.layer.position = CGPointMake(self.frame.size.width/2, self.frame.size.height/2);
...
- (void)reloadData{
    ...
    for (int i = 0 ; i < _idArray.count; i++) {
        [_sourceArray addObject:@{@"id":[NSString stringWithFormat:@"%d",_idArray[i]],
                                 @"title":_titleArray[i],
                                 @"url":_urlArray[i]}];
    }
